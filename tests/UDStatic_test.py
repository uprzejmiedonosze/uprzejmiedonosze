...
class UDStatic_test(unittest.TestCase):
    ...
    def test_changelog(self):
        changelog = StaticPage(self.driver, Locators.CHANGELOG)
        changelog.is_title_matches("istoria"),
        changelog.is_new_matches()
    
    def test_project(self):
        project = StaticPage(self.driver, Locators.PROJECT)
        project.is_title_matches("projekcie")
        project.is_new_matches()

    def test_rtd(self):
        rtd = StaticPage(self.driver, Locators.RTD)
        rtd.is_title_matches("to dobrze")
        rtd.is_new_matches()
    
    def test_przepisy(self):
        przepisy = StaticPage(self.driver, Locators.PRZEPISY)
        przepisy.is_title_matches("rzepisy")
    
    def test_faq(self):
        faq = StaticPage(self.driver, Locators.FAQ)
        faq.is_title_matches("zadawane pytania")
    
    def test_statystyki(self):
        stat = StaticPage(self.driver, Locators.STATYSTYKI)
