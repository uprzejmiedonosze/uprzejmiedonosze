...
if [[ ${PARAM} =~ @ ]]; then
	...
	for key in $(ssh nieradka.net "${SQL} \"select key, json_extract(value, '$.email'), plateId from applications where upper(plateId) upper lower('%${PARAM}%') or lower(json_extract(value, '$.address.address')) like lower('%${PARAM}%') order by json_extract(value, '$.added') desc limit 100 \""); do
